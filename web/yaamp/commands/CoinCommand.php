...
class CoinCommand extends CConsoleCommand
{
    ...
    {
        return dboscalar("SELECT COUNT(*) FROM coins WHERE symbol=:symbol", array(
            ':symbol' => $symbol
        ));
    }
    ...
    public function checkConfirmedBlocks($symbol)
    {
        ...
        $data = $blocks->findAll(array(
            'condition' => "coin_id=:id AND category='generate'",
            'order' => 'height DESC',
            'params' => array(
                ':id' => $coin->id
            )
    ...
    {
        if (count($args) < 3) die("usage: yiimp coin <SYM> get <key>\n");
        $symbol = $args[0];
        $key = $args[2];
    ...
    {
        if (count($args) < 4) die("usage: yiimp coin <SYM> set <key> <value>\n");
        $symbol = $args[0];
        $key = $args[2];
        $value = $args[3];
        if (!$this->checkSymbol($symbol))
    ...
    {
        if (count($args) < 3) die("usage: yiimp coin <SYM> unset <key>\n");
        $symbol = $args[0];
        $key = $args[2];
        if (!$this->checkSymbol($symbol))
        {
    ...
    {
        if (count($args) < 2) die("usage: yiimp coin <SYM> settings\n");
        $symbol = $args[0];
        if (!$this->checkSymbol($symbol))
        {
            echo "error: symbol '$symbol' does not exist!\n";
