...
function controller()
{
    ...
    if (app() instanceof CConsoleApplication) return app();
...
function cache()
{
    if (app() instanceof CConsoleApplication) return app()->cache;

    return app()
        ->getController()->memcache;
}

function objSafeVal($obj, $key, $default = NULL)
{
    if (is_object($obj) && property_exists($obj, $key)) return $obj->$key;
    elseif (is_array($obj)) return arraySafeVal($obj, $key, $default);
    return $default;
}
...
function arraySafeVal($arr, $key, $default = NULL)
{
    ...
    elseif (is_object($arr)) return objSafeVal($arr, $key, $default);
    return $default;
}
...
{
    if (strpos($_SERVER['SERVER_NAME'], ':')) return substr($_SERVER['SERVER_NAME'], 0, strpos($_SERVER['SERVER_NAME'], ':'));
...
{
    if (isset($_SERVER['HTTPS']) && !strcasecmp($_SERVER['HTTPS'], 'on')) $protocol = 'https';
    else $protocol = 'http';
...
function getClientPlatform()
{
    ...
    if (preg_match('/ipad/i', $agent)) $platform = 'Ipad';

    else if (preg_match('/iphone/i', $agent)) $platform = 'Iphone';

    else if (preg_match('/android/i', $agent)) $platform = 'Android';

    else if (preg_match('/linux/i', $agent)) $platform = 'Linux';
...
function IsMobileDevice()
{
    ...
    return preg_match('/ipad/i', $agent) || preg_match('/iphone/i', $agent) || preg_match('/android/i', $agent);
}
...
function file_get_contents_curl($url, $user = null)
{
    ...
    $callback = function ($ch, $text) use (&$data, &$maxallowed, &$totalread)
    {
        ...
        if ($totalread >= $maxallowed || stristr($data, '</head>')) return 0;
