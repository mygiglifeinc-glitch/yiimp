...
function NotifyCheckRules()
{
    ...
    $notifications = $search->with('coin')
        ->findAll("enabled AND lastchecked <= {$since} " . " AND (nc.installed OR nc.enable OR nc.watch)");
    ...
    foreach ($notifications as $rule)
    {
        ...
        switch ($rule->notifytype)
        {
            case 'email':
                ...
                if (!$res) debuglog("notify: {$coin->symbol} unable to send mail to {$dest}!");
                break;
            ...
            case 'rpc':
                ...
                if ($res === false) debuglog("trigger: {$coin->symbol} rpc error '{$command}' {$remote->error}");
                else debuglog("trigger: {$coin->symbol} rpc -> $res");
                break;
