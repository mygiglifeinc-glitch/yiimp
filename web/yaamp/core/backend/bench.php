...
function BenchUpdateChips()
{
    ...
    foreach ($benchs as $bench) {
        ...
        $dups = getdbocount('db_benchmarks', "vendorid=:vid AND client=:client AND os=:os AND driver=:drv AND throughput=:thr AND userid=:uid", array(
            ':vid' => $bench->vendorid,
            ':client' => $bench->client,
            ':os' => $bench->os,
            ':drv' => $bench->driver,
            ':thr' => $bench->throughput,
            ':uid' => $bench->userid
        ));
        ...
        if (!empty($chip) && $chip != '-') {
            $bench->chip = $chip;
            $rates       = dborow("SELECT AVG(khps) AS avg, COUNT(id) as cnt FROM benchmarks WHERE algo=:algo AND chip=:chip", array(
                ':algo' => $bench->algo,
                ':chip' => $chip
            ));
            $avg         = (float) $rates['avg'];
    ...
    foreach ($rows as $row) {
        if (empty($row['chip']) || empty($row['type']))
            continue;
        $chip = getdbosql('db_bench_chips', 'chip=:name AND devicetype=:type', array(
            ':name' => $row['chip'],
            ':type' => $row['type']
        ));
        if (!$chip || !$chip->id)
