...
function BackendStatsUpdate()
{
    ...
    foreach (yaamp_get_algos() as $algo) {
        ...
        $stats = getdbosql('db_hashstats', "time=$tm and algo=:algo", array(
            ':algo' => $algo
        ));
        if (!$stats) {
        ...
        $earnings = bitcoinvaluetoa(dboscalar("SELECT SUM(amount*price) FROM blocks WHERE algo=:algo AND time>$tm AND category!='orphan'", array(
            ':algo' => $algo
        )));
    ...
    foreach (yaamp_get_algos() as $algo) {
        $stats = getdbosql('db_hashrate', "time=$tm and algo=:algo", array(
            ':algo' => $algo
        ));
        if (!$stats) {
            $stats               = new db_hashrate;
            $stats->time         = $tm;
            $stats->hashrate     = dboscalar("select hashrate from hashrate where algo=:algo order by time desc limit 1", array(
                ':algo' => $algo
            ));
            ...
            $stats->price        = dboscalar("select price from hashrate where algo=:algo order by time desc limit 1", array(
                ':algo' => $algo
            ));
            $stats->rent         = dboscalar("select rent from hashrate where algo=:algo order by time desc limit 1", array(
                ':algo' => $algo
            ));
            $stats->algo         = $algo;
        }
        ...
        $t1             = time() - 5 * 60;
        $total_rentable = dboscalar("select sum(difficulty) from shares where valid and extranonce1 and algo=:algo and time>$t1", array(
            ':algo' => $algo
        ));
        $total_diff     = dboscalar("select sum(difficulty) from shares where valid and algo=:algo and time>$t1", array(
            ':algo' => $algo
        ));
        $total_rented   = 0;
        ...
        if ($total_diff > 0) {
            ...
            $list = dbolist("select coinid, sum(difficulty) as d from shares where valid and algo=:algo and time>$t1 group by coinid", array(
                ':algo' => $algo
            ));
            ...
            $bb = dboscalar("select sum(speed) from jobs where active and ready and price>$rent and algo=:algo", array(
                ':algo' => $algo
            ));
            ...
            $coin = getdbosql('db_coins', "enable and auto_ready and algo=:algo order by index_avg desc", array(
                ':algo' => $algo
            ));
            if ($coin) {
                $btcghd       = yaamp_profitability($coin);
        ...
        if (YAAMP_LIMIT_ESTIMATE) {
            $t1  = time() - 24 * 60 * 60;
            $avg = dboscalar("select avg(price) from hashrate where time>$t1 and algo=:algo", array(
                ':algo' => $algo
            ));
            if ($avg)
        ...
        $stats->difficulty = dboscalar("select sum(difficulty) from coins where enable and auto_ready and algo=:algo", array(
            ':algo' => $algo
        ));
        $stats->save();
    }
    ...
    if (!$btc)
        $btc = json_decode('{"id": 6, "balance": 0}');
    ...
    foreach (yaamp_get_algos() as $algo) {
        ...
        $dbalgo->profit = dboscalar("select price from hashrate where algo=:algo order by time desc limit 1", array(
            ':algo' => $algo
        ));
        $dbalgo->rent   = dboscalar("select rent from hashrate where algo=:algo order by time desc limit 1", array(
            ':algo' => $algo
        ));
...
function BackendStatsUpdate2()
{
    ...
    foreach ($list as $item) {
        $stats = getdbosql('db_hashuser', "time=$tm and algo=:algo and userid=:userid", array(
            ':algo' => $item['algo'],
            ':userid' => $item['userid']
        ));
        if (!$stats) {
            ...
            $stats->time         = $tm;
            $stats->hashrate     = dboscalar("select hashrate from hashuser where algo=:algo and userid=:userid order by time desc limit 1", array(
                ':algo' => $item['algo'],
                ':userid' => $item['userid']
            ));
    ...
    foreach ($list as $item) {
        ...
        if (!$stats) {
            ...
            $stats->time         = $tm;
            $stats->hashrate     = dboscalar("select hashrate from hashrenter where jobid=:jobid order by time desc limit 1", array(
                ':jobid' => $jobid
            ));
