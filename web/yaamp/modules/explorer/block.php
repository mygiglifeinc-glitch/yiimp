...
if (isset($block['flags'])) echo '<tr><td>Flags:</td><td><span class="monospace">' . $block['flags'] . '</span></td></tr>';

if (isset($block['previousblockhash']) && $coin->algo == 'x16r')
{
    echo '<tr><td>Hash order:</td><td><span class="monospace">' . substr($block['previousblockhash'], -16) . '</span></td></tr>';
}

if (isset($block['previousblockhash'])) echo '<tr><td>Previous Hash:</td><td><span class="monospace">' . $coin->createExplorerLink($block['previousblockhash'], array(
    'hash' => $block['previousblockhash']
)) . '</span></td></tr>';

if (isset($block['nextblockhash'])) echo '<tr><td>Next Hash:</td><td><span class="monospace">' . $coin->createExplorerLink($block['nextblockhash'], array(
...
foreach ($block['tx'] as $txhash)
{
    ...
    if ($segwit) echo '&nbsp;<img src="/images/ui/segwit.png" height="8px" valign="center" title="segwit"/>';
    echo "</td>";

    echo "<td>";
    $nvout = count($tx['vout']);;
    if ($nvout > 500) echo "Too much addresses to display ($nvout)";
    else foreach ($tx['vout'] as $vout)
    {
        $value = $vout['value'];
        if ($value == 0) continue;
...
if ($coin->rpcencoding == 'DCR' && isset($block['stx']))
{
    ...
    foreach ($block['stx'] as $txhash)
    {
        ...
        echo "<td>";
        if (isset($stx['vout'][0]['scriptPubKey']) && arraySafeVal($stx['vout'][0]['scriptPubKey'], 'type') == 'stakesubmission') echo "Ticket";
        else foreach ($stx['vin'] as $vin)
        {
            if (arraySafeVal($vin, 'blockheight') > 0)
            {
                echo $coin->createExplorerLink($vin['blockheight'], array(
                    'height' => $vin['blockheight']
                ));
                echo '<br/>';
            }
        }
