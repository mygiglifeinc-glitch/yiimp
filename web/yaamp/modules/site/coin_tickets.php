...
if (!empty($txs))
{
    ...
    {
        if (arraySafeVal($tx, 'time', time()) > $list_since) $txs_array[] = $tx;
    }
    krsort($txs_array);
}
...
if (!empty($txs_array))
{
    ...
    foreach ($txs_array as $key => $tx)
    {
        ...
        else if ($category == 'send' && $prev_txid === arraySafeVal($tx, "txid"))
        {
            ...
            if ($prev_tx['amount'] == 0 - $tx['amount']) $txs_array[$key]['category'] = 'spent';
        }
...
if (!empty($tickets)) foreach ($tickets['hashes'] as $n => $txid)
{
    if (!in_array($txid, $list_txs))
    {
        ...
        $stx['category'] = 'ticket';
        if (isset($stx['vin'][0])) $stx['input'] = $stx['vin'][0]['amountin'] * $amountin_mul;
        $commitamt = 0.;
        foreach ($stx['vout'] as $v)
        {
            if (arraySafeVal($v['scriptPubKey'], 'type') == 'sstxcommitment') $commitamt += $v['scriptPubKey']['commitamt'];
        }
...
foreach ($txs_array as $tx)
{
    ...
    if ($block) echo '<td>' . $block['height'] . '</td>';
    else echo '<td></td>';
