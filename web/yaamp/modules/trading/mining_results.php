...
$res = false;
if (isset($table[$algo]) && YAAMP_USE_NICEHASH_API) $res = fetch_url("https://api.nicehash.com/api?method=orders.get&algo={$table[$algo]}");
...
$nicehash = getdbosql('db_nicehash', "algo=:algo and orderid!=0", array(
    ':algo' => $algo
));
if ($nicehash)
{
...
foreach ($list as $coin)
{
    ...
    if (!$coin->auto_ready) echo "<tr style='opacity: 0.4;'>";
    ...
    if (!empty($coin->errors)) echo "<td align=right style='font-size: .8em; color: red;' title='$coin->errors'>$height</td>";
    else echo "<td align=right style='font-size: .8em;'>$height</td>";
...
$hashrate = dboscalar("select sum(difficulty) * $target / $interval / 1000 from shares where valid and time>$delay and
	workerid in (select id from workers where algo=:algo and version='$version')", array(
    ':algo' => $algo
));
...
$count = getdbocount('db_workers', "algo=:algo and version='$version'", array(
    ':algo' => $algo
));
...
function show_orders(&$allorders, &$services, $btcmhd = 0)
{
    ...
    foreach ($allorders as $i => $order)
    {
        ...
        if (isset($order['me'])) echo "<tr class='ssrow' style='background-color: #dfd'>";
        else echo "<tr class='ssrow'>";
